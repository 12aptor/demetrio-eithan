...
function draw_star(x, y, maxRadius, baseColor) {
  ctx.save();
...
function draw_spike(x, y, length, width, angle) {
  ctx.save();
  ctx.translate(x, y);
  ctx.rotate(angle);
...
var STAR_COUNT = 150;
var Z_OFFSET = 4.2;
var FOCAL_LENGTH = 1.45;
var BASE_IMG_SIZE = 0.32;
...
function load_thumbnail_async(thumbUrl, fallbackUrl) {
  if (imgCache[thumbUrl] || loadingUrls.has(thumbUrl))
    return;
...
function draw_placeholder(x, y, w, h, z) {
  ctx.save();
  const rx = x - w / 2;
  const ry = y - h / 2;
...
function show_modal(thumbUrl, fullUrl) {
  if (activeModal)
    return;
...
function handle_click(mx, my) {
  ...
  for (const v of sortedNearToFar) {
    ...
    if (mx >= pos.x - targetWidth / 2 && mx <= pos.x + targetWidth / 2 && my >= pos.y - targetHeight / 2 && my <= pos.y + targetHeight / 2) {
...
function draw_ring_image(ringImg) {
  ...
  if (pos.x + targetWidth / 2 < 0 || pos.x - targetWidth / 2 > canvas.width || pos.y + targetHeight / 2 < 0 || pos.y - targetHeight / 2 > canvas.height) {
    return;
  }
  if (img) {
...
function frame(now) {
  ...
  if (!isDragging) {
    if (Math.abs(scrollVelocity) > 0.01) {
      ...
      const speedMultiplier = 1 + 0.75 * Math.sin(now * 0.0008) * Math.cos(now * 0.0003);
